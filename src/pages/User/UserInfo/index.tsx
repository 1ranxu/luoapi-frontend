...
} from "@/services/luoapi-backend/userController";
import Settings from '../../../../config/defaultSettings';
import Paragraph from "antd/lib/typography/Paragraph";
import ProCard from "@ant-design/pro-card";
import {requestConfig} from "@/requestConfig";
import EmailModal from "@/components/EmailModal";
import {doSignInUsingPost} from "@/services/luoapi-backend/signInController";
...
const UserInfo: React.FC = () => {
  ...
  const updateUserInfo = async () => {
    ...
    if (res.data && res.code === 0) {
      ...
      setInitialState({loginUser: loginUser.data, settings: Settings})
      ...
      >
        ...
        >
          ...
          {<Button loading={dailyCheckInLoading}
            ...
            const res = await doSignInUsingPost()
            if (res.data && res.code === 0) {
              const res = await getLoginUserUsingGet();
              if (res.data && res.code === 0) {
        ...
        >
          <Button size={"large"}>
            <a target={"_blank"} href={"https://github.com/1ranxu/luoapi-client-sdk"}
